...
use crate::normalize::{normalize_ref, ref_view_for, RefSeq, RefView};
...
///   already be resident in `store` (e.g. via `RefgetStore::load_sequence`
///   then `into_readonly`); the reference is read through the canonical
///   [`RefView`], which decodes 2-bit-encoded stores on the fly, so no
///   eager full decode is required. Sidecar chroms with a zero-digest
    ...
    F: FnMut(VrsRow) + Send,
{
    ...
    // Build per-chrom work units. The reference is exposed through the
    // canonical `RefView` (built once per chrom), which borrows the store's
    // bytes and decodes 2-bit-encoded stores on the fly. `RefView: Sync`, so
    // `&[ChromWork]` is shareable across the scoped workers.
    struct ChromWork<'a> {
        name: &'a str,
        view: RefView<'a>,
    ...
    for (idx, c) in reader.chromosomes().iter().enumerate() {
        ...
        let view = ref_view_for(store, c.seq_digest.as_str()).with_context(|| {
            format!(
                "chrom {} (digest {}) not resident in store",
                c.name, c.seq_digest
            )
        })?;
                    ...
                    for res in iter {
                        ...
                        match rec {
                            RgvcfRecord::Snv { pos, alt, .. } => {
                                ...
                                let ref_b = w.view.base_at(idx);
                                let ref_buf = [ref_b];
                                let alt_buf = [alt];
                                let norm = match normalize_ref(&w.view, start0, &ref_buf, &alt_buf) {
                                    Ok(n) => n,
                                    Err(_) => continue,
                                };
                            ...
                            RgvcfRecord::Complex { pos, ref_, alts, .. } => {
                                ...
                                for alt in alts.iter() {
                                    ...
                                    let norm = match normalize_ref(&w.view, start0, ref_, alt) {
                                        Ok(n) => n,
                                        Err(_) => continue,
                                    };
                                    let norm_seq = match std::str::from_utf8(&norm.allele) {
