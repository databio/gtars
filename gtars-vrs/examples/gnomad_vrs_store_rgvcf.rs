...
fn main() -> Result<()> {
    ...
    // Read rgvcf header (cheap) to learn which chromosomes to make resident.
    // The reference is decoded on the fly via `RefView`, so loading the
    // (possibly 2-bit-encoded) sequence bytes is sufficient — no eager decode.
