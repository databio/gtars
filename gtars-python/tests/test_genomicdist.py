...
class TestRegionSetStatistics:
    ...
    def test_distribution(self):
        ...
        result = rs.distribution(10)
        assert isinstance(result, dict)
        assert "bins" in result and "out_of_range" in result
        bins = result["bins"]
        assert isinstance(bins, list)
        assert all(isinstance(d, dict) for d in bins)
        assert all(k in d for d in bins for k in ("chr", "start", "end", "n", "rid"))
        # At least some bins should have n > 0
        assert any(d["n"] > 0 for d in bins)
        # Without chrom_sizes, out_of_range is always empty
        assert result["out_of_range"] == {}

    def test_distribution_default_bins(self):
        rs = make_regionset([("chr1", 0, 1000)])
        result = rs.distribution()
    ...
    def test_distribution_with_chrom_sizes(self):
        ...
        result = rs.distribution(10, chrom_sizes)
        bins = result["bins"]
        assert isinstance(bins, list)
        assert all(isinstance(d, dict) for d in bins)
        # chr1 bin width should be 100 (1000/10)
        chr1_bins = [d for d in bins if d["chr"] == "chr1"]
        assert all(d["end"] - d["start"] == 100 for d in chr1_bins)
        # chr2 bin width should be 50 (500/10)
        chr2_bins = [d for d in bins if d["chr"] == "chr2"]
        assert all(d["end"] - d["start"] == 50 for d in chr2_bins)
        # All regions fit — no out-of-range
        assert result["out_of_range"] == {}

    def test_distribution_out_of_range_reporting(self):
        """Regions beyond stated chrom_size should be skipped and reported."""
        # chr1 size 1000, region at [1200, 1300] has midpoint 1250 > 1000 (out of range)
        # chr2 size 500, regions at [200, 300] (in range) and [2000, 2100] (out of range)
        rs = make_regionset([
        ...
        chrom_sizes = {"chr1": 1000, "chr2": 500}
        result = rs.distribution(10, chrom_sizes)
        # 2 of 4 regions should be in bins
        total_in_bins = sum(d["n"] for d in result["bins"])
        assert total_in_bins == 2
        # out_of_range should report both chromosomes
        assert result["out_of_range"] == {"chr1": 1, "chr2": 1}
        # All bin ends should be > bin starts (no old-bug artifacts)
        assert all(d["end"] > d["start"] for d in result["bins"])

    def test_distribution_without_chrom_sizes_uses_bed_derived(self):
        """Without chrom_sizes, bin size is derived from BED's max end coord."""
        rs = make_regionset([("chr1", 0, 100), ("chr2", 500, 1000)])
        # max_end observed = 1000, 10 bins → bin_size = 100
        result = rs.distribution(10)
        bins = result["bins"]
        # All bins across chroms should have the same width (BED-derived)
        widths = set(d["end"] - d["start"] for d in bins)
        # Last (partial) bin can be narrower; non-last bins should all be 100
