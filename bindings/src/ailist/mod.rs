...
use genimtools::ailist::{Interval, AIList};
...
impl PyAIList {
    ...
    fn new(py_interval_list: Vec<PyRef<PyInterval>>, minimum_coverage_length: Option<usize>) -> PyAIList {
            let mut interval_list: Vec<Interval> = py_interval_list
                .into_iter()
                .map(|x| Interval{start: x.start, end: x.end}).collect();
    ...
    fn query(&self, py_interval: &PyInterval) -> Vec<PyInterval> {
        let interval: Interval = Interval{ start: py_interval.start, end: py_interval.end };
