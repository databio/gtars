use gtars::scatrs::models::{ScatrsRegion, FragmentDistribution};
use gtars::scatrs::sampling::WeightedSampler;
use rand::prelude::*;

#[test]
fn test_wrswr_skip_integration() {
    // Create test data
...
#[test]
fn test_wrswr_performance_comparison() {
    ...
    for _ in 0..10 {
        ...
        let _ = sampler.sample_regions_for_cell_optimized(
            ...
            &weights,
            0.5,
            &FragmentDistribution::Uniform { count: 1000 },
            &mut local_rng,
        );
    }
    ...
    println!("  WRSWR-SKIP time for 10 iterations: {:?}", optimized_time);
...
fn test_without_replacement_property() {
    ...
    let result = sampler.sample_regions_for_cell_optimized(
        ...
        &FragmentDistribution::Uniform { count: 50 },
        &mut rng,
    ).unwrap();
