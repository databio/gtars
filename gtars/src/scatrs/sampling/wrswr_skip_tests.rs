use super::{WRSWRSkipSampler, OptimizedWRSWRSkip};
...
mod statistical_tests {
    ...
    fn test_sampling_distribution_uniform() {
        ...
        for _ in 0..num_trials {
            let mut sampler = WRSWRSkipSampler::new(k, None);
    ...
    fn test_deterministic_with_seed() {
        ...
        // First run
        let mut sampler1 = WRSWRSkipSampler::new(k, Some(seed));
        ...
        let mut sampler2 = WRSWRSkipSampler::new(k, Some(seed));
    ...
    fn test_edge_cases() {
        ...
        // k = 0
        let items: Vec<usize> = vec![1, 2, 3];
        let mut sampler = WRSWRSkipSampler::new(0, Some(42));
        let result = sampler.sample_stream(items.into_iter(), |_| 1.0);
        assert_eq!(result.len(), 0);
        ...
        let mut sampler = WRSWRSkipSampler::new(2, Some(42));
    ...
    fn test_optimized_version_equivalence() {
        ...
        for trial in 0..num_trials {
            ...
            // Standard version
            let mut sampler = WRSWRSkipSampler::new(k, Some(seed));
            let result = sampler.sample_stream(
                items.iter().zip(weights.iter()),
                |(_, &weight)| weight
            );
            ...
            // Optimized version
            let mut sampler = OptimizedWRSWRSkip::new(k, Some(seed));
            let result = sampler.sample_stream(
                items.iter().zip(weights.iter()),
                |(_, &weight)| weight
            );
    ...
    fn test_batch_sampling() {
        ...
        let batches = WRSWRSkipSampler::sample_batch(
    ...
    fn test_streaming_property() {
        ...
        // Process all at once
        let mut sampler1 = WRSWRSkipSampler::new(k, Some(42));
        let result1 = sampler1.sample_stream(
            items.iter().zip(weights.iter()),
            |(_, &weight)| weight
        );
        ...
        let mut sampler2 = WRSWRSkipSampler::new(k, Some(42));
