...
use crate::refget::digest::{sha512t24u, canonicalize_json};
...
impl SeqColDigestLvl1 {
    ...
    pub fn to_digest(&self) -> String {
        ...
        lvl1_object.insert("names".to_string(), serde_json::Value::String(self.names_digest.clone()));
        lvl1_object.insert("sequences".to_string(), serde_json::Value::String(self.sequences_digest.clone()));
    ...
    pub fn from_metadata(metadata_vec: &[&SequenceMetadata]) -> Self {
        ...
        let sequences: Vec<String> = metadata_vec.iter().map(|md| format!("SQ.{}", md.sha512t24u)).collect();
        ...
        let sequences_json = Value::Array(sequences.iter().map(|s| Value::String(s.to_string())).collect());
        let names_json = Value::Array(names.iter().map(|s| Value::String(s.to_string())).collect());
        let lengths_json = Value::Array(lengths.iter().map(|l| Value::Number(serde_json::Number::from(*l))).collect());
...
impl SequenceCollection {
    ...
    pub fn from_farg<P: AsRef<Path>>(file_path: P) -> Result<Self> {
        ...
        if farg_file_path.exists() {
            ...
            Err(anyhow::anyhow!("FARG file does not exist at {:?}", farg_file_path))
        }
    }
    ...
    pub fn from_path_with_cache<P: AsRef<Path>>(
        ...
        println!("from path with cache: reading from file: {:?}", file_path.as_ref());
        println!("Farg file path: {:?}", farg_file_path);
        ...
        if write_cache && !farg_file_path.exists() {
            ...
            println!("Farg file already exists, not writing: {:?}", farg_file_path);
        }
        Ok(seqcol)
    }
...
impl Display for SequenceCollection {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "SequenceCollection with {} sequences, digest: {}", self.sequences.len(), self.digest)?;
        write!(f, "\nFirst 3 sequences:")?;
