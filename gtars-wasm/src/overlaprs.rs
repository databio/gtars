...
use gtars_overlaprs::{Overlapper, AiList, Bits};
...
impl JsOverlapper {
    ...
    pub fn new(universe: &JsValue, backend: String) -> Result<JsOverlapper, JsValue> {
        ...
        match backend.as_str() {
            ...
            _ => {
                Err(JsValue::from_str(&format!("Invalid backend specified: {}", backend)))
            }
        }
    }
    ...
    pub fn intersect(&self, regions: &JsValue) -> Result<BedEntries, JsValue> {
        ...
        let tokens = regions.0.into_iter().map(|e| {
            let chrom = e.0;
            let start = e.1;
            let end = e.2;
            ...
            match olapper {
                Some(olapper) => {
                    Some(olapper.find_iter(start, end)
