...
use gtars::scatrs::sampling::{WeightedSampler, a_res::{AResSampler, VectorAResSampler}};
use rand::prelude::*;
...
fn benchmark_a_res(c: &mut Criterion) {
    let mut group = c.benchmark_group("a_res_sampling");
    ...
    for (n, k) in test_cases {
        ...
        // Benchmark A-Res (heap-based)
        group.bench_with_input(
            BenchmarkId::new("a_res_heap", format!("n={}_k={}", n, k)),
            &(&regions, &weights, k),
            |b, (regions, weights, k)| {
                b.iter(|| {
        ...
        // Benchmark Vector-based A-Res
        group.bench_with_input(
            BenchmarkId::new("a_res_vector", format!("n={}_k={}", n, k)),
            &(&regions, &weights, k),
            |b, (regions, weights, k)| {
                b.iter(|| {
                    let mut sampler = VectorAResSampler::new(*k, Some(42));
                    sampler.sample_stream(
                        regions.iter().cloned().zip(weights.iter().cloned()),
                        |(_, weight)| *weight
                    )
...
fn benchmark_batch_sampling(c: &mut Criterion) {
    ...
    for num_batches in [1, 10, 100, 1000] {
        group.bench_with_input(
            BenchmarkId::new("batch_a_res", format!("batches={}", num_batches)),
            &(&regions, &weights, k, num_batches),
            |b, (regions, weights, k, num_batches)| {
                b.iter(|| {
                    AResSampler::sample_batch(
